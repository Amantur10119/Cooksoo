    ...
    <header className="fixed backdrop-blur-md z-50 inset-x-0 top-0 flex items-center justify-between px-12 md:px-20 py-6">
      ...
      <nav className="flex items-center justify-center gap-8">
        ...
        >
          ...
          {cart?.length > 0 && (
            <div className="w-6 h-6 rounded-full bg-purple-500 flex items-center justify-center absolute -top-4 -right-1">
        ...
        {user ? (
              ...
              {isMenu && (
                ...
                >
                  {user?.user_id === process.env.REACT_APP_ADMIN_ID && (
                    <Link
                      className=" hover:text-purple-500 text-xl text-textColor"
                      to={"/dashboard/home"}
                    >
                      Dashboard
                    </Link>
          ...
          <>
            <NavLink to={"/login"}>
              <motion.button
                {...buttonClcik}
                className="px-4 py-2 rounded-md shadow-md bg-lightOverlay border border-purple-300 cursor-pointer"
              >
                Login
              </motion.button>
            </NavLink>
